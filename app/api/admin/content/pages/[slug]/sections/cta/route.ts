...
import {
  ...
  type CtaSectionMutationRequest,
...
function newCtaSectionKey(slug: string, currentKeys: string[]): string {
  const base = `cta-${slug}`.slice(0, 76);
...
function ctaSection(input: CtaSectionMutationRequest, key: string): PageSection {
  return {
    key,
    type: 'cta',
    active: input.active,
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  let body: Partial<CtaSectionMutationRequest> | null;
  try {
    body = (await request.json()) as Partial<CtaSectionMutationRequest>;
  } catch {
    return NextResponse.json<ContentSectionMutationResponse>(
  ...
  const validation = validateCtaSection(body);
  if (!validation.valid) {
    return NextResponse.json<ContentSectionMutationResponse>(
  ...
  if (existingSection && existingSection.type !== 'cta') {
    return NextResponse.json<ContentSectionMutationResponse>(
      { message: 'Selected section is not a CTA section.' },
      { status: 400 },
    );
  }
  ...
  const nextSection = ctaSection(validation.data, sectionKey);
  if (existingIndex >= 0) {
  ...
  return NextResponse.json<ContentSectionMutationResponse>(
    {
      message: existingIndex >= 0 ? 'CTA section saved.' : 'CTA section added.',
