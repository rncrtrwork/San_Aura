...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const item = await ResortRule.findById(itemId).select(
  ...
  const duplicate = await ResortRule.exists({
    _id: { $ne: item._id },
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  await connectToDatabase();
  const item = await ResortRule.findById(itemId).select('category title slug status displayOrder');
  if (!item) {
    return NextResponse.json<ManagedContentItemResponse>(
      { message: 'Resort rule not found.' },
      { status: 404 },
