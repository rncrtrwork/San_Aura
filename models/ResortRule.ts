...
export type ResortRuleDocument = {
  category: string;
  title: string;
  slug: string;
  body: string;
...
const resortRuleSchema = new Schema<ResortRuleDocument>(
  {
...
resortRuleSchema.index({ category: 1, displayOrder: 1 });

export const ResortRule =
  (models.ResortRule as Model<ResortRuleDocument> | undefined) ??
  model<ResortRuleDocument>('ResortRule', resortRuleSchema);
